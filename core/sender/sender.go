...
func SendRequest(node string, index int) {
	...
	if err != nil {
		console.Log(fmt.Sprintf("Failed in node #%v%d%v", console.PrimaryColor, index, console.SecondaryColor), true)
		return
	}
	...
	if strings.Contains(body, "be acted upon") || strings.Contains(body, "Confirmation from your email") {
		console.Log(fmt.Sprintf("Sent in node #%v%d%v", console.PrimaryColor, index, console.SecondaryColor), true)
	} else if LogFailed {
		if strings.Contains(body, "is banned") {
			console.Log(fmt.Sprintf("Email is banned node #%v%d%v", console.PrimaryColor, index, console.SecondaryColor), true)
